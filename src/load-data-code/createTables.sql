...
CREATE VIEW Actors AS SELECT imdb_title_id, imdb_name_id, name FROM MovieRoles INNER JOIN Names USING (imdb_name_id) WHERE played_role = 'actor' OR played_role = 'actress';

-- Create view command for the view Directors
CREATE VIEW Directors AS SELECT imdb_title_id, imdb_name_id, name FROM MovieRoles INNER JOIN Names USING (imdb_name_id) WHERE played_role = 'director';

-- Create view command for the view Writers
CREATE VIEW Writers AS SELECT imdb_title_id, imdb_name_id, name FROM MovieRoles INNER JOIN Names USING (imdb_name_id) WHERE played_role = 'writer';
