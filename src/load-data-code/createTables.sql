...
CREATE TABLE Movies(imdb_title_id varchar(12), title varchar(100), original_title varchar(100), year_of_release decimal(4,0), date_published datetime, duration int, country varchar(50), language varchar(50), director varchar(200), writer varchar(200), actors varchar(500), production_company varchar(100), description varchar(8000), budget varchar(25), usa_gross_income varchar(25), worlwide_gross_income varchar(25), reviews_from_users int, reviews_from_critics int, PRIMARY KEY (imdb_title_id));
...
GO
CREATE VIEW Actors AS SELECT imdb_title_id, imdb_name_id, name FROM MovieRoles INNER JOIN Names ON MovieRoles.imdb_name_id = Names.imdb_name_id WHERE played_role = 'actor' OR played_role = 'actress';

-- Create view command for the view Directors
GO
CREATE VIEW Directors AS SELECT imdb_title_id, imdb_name_id, name FROM MovieRoles INNER JOIN Names ON MovieRoles.imdb_name_id = Names.imdb_name_id WHERE played_role = 'director';

-- Create view command for the view Writers
GO
CREATE VIEW Writers AS SELECT imdb_title_id, imdb_name_id, name FROM MovieRoles INNER JOIN Names ON MovieRoles.imdb_name_id = Names.imdb_name_id WHERE played_role = 'writer';
