...
SELECT DISTINCT T1.Name AS 'Name', Ratings.weighted_average_vote AS 'Rating' FROM Ratings INNER JOIN (SELECT Movies.title AS 'Name', Genres.genre AS 'genre', Movies.imdb_title_id AS 'imdb_title_id' FROM Movies INNER JOIN Genres ON Movies.imdb_title_id = Genres.imdb_title_id) AS T1 ON T1.imdb_title_id = Ratings.imdb_title_id WHERE T1.genre like '%Drama%' ORDER BY Ratings.weighted_average_vote DESC LIMIT 10;
...
SELECT Movies.title AS Title, Ratings.us_voters_votes AS Votes FROM Movies INNER JOIN Ratings ON Movies.imdb_title_id = Ratings.imdb_title_id ORDER BY Ratings.us_voters_votes DESC LIMIT 10;

-- 6 Most voted n movies overall - input(n) - output(all movie details)
SELECT Movies.title AS Title, Ratings.total_votes AS Rating FROM Movies INNER JOIN Ratings ON Movies.imdb_title_id = Ratings.imdb_title_id ORDER BY Ratings.total_votes DESC LIMIT 10;

-- 7 Top movies released in x country - input(countryName) - output (all movie details)
SELECT Movies.title AS Title, Ratings.weighted_average_vote AS Rating FROM Movies INNER JOIN Ratings ON Movies.imdb_title_id = Ratings.imdb_title_id WHERE Movies.country LIKE '%USA%' ORDER BY Ratings.weighted_average_vote DESC LIMIT 250;

-- 8 Lowest rated movies - input() - output(movie details exclude null) 
SELECT Movies.title AS Title, Ratings.weighted_average_vote AS Rating FROM Movies INNER JOIN Ratings ON Movies.imdb_title_id = Ratings.imdb_title_id ORDER BY Ratings.weighted_average_vote LIMIT 250;
